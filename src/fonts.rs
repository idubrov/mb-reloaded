...
pub fn load_font<'t>(
  ...
  let texture = load_font_internal(texture_creator, path).map_err(|source| FontLoadingFailed {
    path: path.to_owned(),
    source,
  })?;
  Ok(Font {
...
fn load_font_internal<'t>(
  ...
  let mut texture = texture_creator.create_texture_static(PixelFormatEnum::RGBA32, 16 * 8, 16 * 8)?;
